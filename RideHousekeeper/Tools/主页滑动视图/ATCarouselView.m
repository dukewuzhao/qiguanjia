...
- (void)layoutSubviews {
    ...
    for (int i = 0; i < imageBtnCount; i++) {
        ...
        if (self.isScrollDorectionPortrait) { //竖向
            ...
            imageBtn.frame = CGRectMake(i * width, 0, ScreenWidth *.5, ScreenWidth *.6);
            
            if (LL_iPhoneX) {
                imageBtn.height = ScreenWidth *.7;
            }
...
- (void)setImages:(NSArray *)images {
    ...
    for (int i = 0; i< [LVFmdbTool queryBikeData:nil].count; i++) {
        ...
        if ([bikemodel.mac isEqualToString:[[NSUserDefaults standardUserDefaults]valueForKey:SETRSSI]]) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    self.pageindex = self.pageControl.currentPage;
}
//开始拖拽的时候停止计时器
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (scrollView.contentOffset.x == 0 || scrollView.contentOffset.x == self.bounds.size.width *2) {
        ...
        if ([self.delegate respondsToSelector:@selector(carouselView:indexOfscrollview:)])
        {
            [self.delegate carouselView:self indexOfscrollview:self.pageindex];
        }
